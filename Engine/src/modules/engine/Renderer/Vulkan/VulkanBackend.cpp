...
bool VulkanBackend::selectPhysicalDevice() {
    ...
    for (unsigned int i = 0; i < deviceCount; i++) {
        ...
        if (result) {
            ...
            vulkanContext.getDevice().physicalDevice = devices[i];
...
void VulkanBackend::querySwapChainSupport(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface, VulkanSwapChainSupportInfo& swapChainSupportInfo) {
    ...
    if (swapChainSupportInfo.formats) {
        FF_Memory::ff_free(swapChainSupportInfo.formats,sizeof(VkSurfaceFormatKHR) * swapChainSupportInfo.formatCount,RENDER);
        swapChainSupportInfo.formats = nullptr;
        swapChainSupportInfo.formatCount = 0;
    }
    if (swapChainSupportInfo.presentModes) {
        FF_Memory::ff_free(swapChainSupportInfo.presentModes,sizeof(VkPresentModeKHR) * swapChainSupportInfo.presentCount,RENDER);
        swapChainSupportInfo.presentModes = nullptr;
        swapChainSupportInfo.presentCount = 0;
    }
...
void VulkanBackend::destroySwapchain() {
    ...
    if (vulkanContext.getSwapchain().imageViews) {
        for (unsigned int i = 0; i < vulkanContext.getSwapchain().imageCount; i++) {
            if (vulkanContext.getSwapchain().imageViews[i]) {
                vkDestroyImageView(vulkanContext.getDevice().logicalDevice, vulkanContext.getSwapchain().imageViews[i], nullptr);
                vulkanContext.getSwapchain().imageViews[i] = nullptr;
            }
        }

        FF_Memory::ff_free(vulkanContext.getSwapchain().imageViews, sizeof(VkImageView) * vulkanContext.getSwapchain().imageCount, RENDER);
        vulkanContext.getSwapchain().imageViews = nullptr;
    }

    if (vulkanContext.getSwapchain().images) {
        FF_Memory::ff_free(vulkanContext.getSwapchain().images, sizeof(VkImage) * vulkanContext.getSwapchain().imageCount, RENDER);
        vulkanContext.getSwapchain().images = nullptr;
    }

    if (vulkanContext.getSwapchain().handle) {
        vkDestroySwapchainKHR(vulkanContext.getDevice().logicalDevice, vulkanContext.getSwapchain().handle, nullptr);
        vulkanContext.getSwapchain().handle = nullptr;
    }
