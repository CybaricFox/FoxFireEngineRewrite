...
Keys PlatformLinux::translateKeycode(const unsigned int keyCode) {
    switch (keyCode) {
        case XK_BackSpace:
...
void PlatformLinux::printConsoleMessage(const String &message, unsigned char color) {
    String output;
...
void PlatformLinux::printConsoleError(const String &message, unsigned char color) {
    String output;
...
bool PlatformLinux::createSurface() {
...
bool PlatformLinux::processMessages() {
    ...
    while ((event = xcb_poll_for_event(connection)) != nullptr) {
        ...
        switch (event->response_type & ~0x80) {
            ...
            case XCB_CLIENT_MESSAGE: {
                ...
                //Close window
                if (message->data.data32[0] == wm_delete_win) {
                    quitFlag = true;
                }
                break;
            }
...
void PlatformLinux::getRequiredExtensions(DynamicArray<const char *> &extensions) {
    extensions.push("VK_KHR_xcb_surface");
}

void PlatformLinux::ff_sleep(unsigned long ms) {
...
bool PlatformLinux::initialize(const String &applicationName, const int x, const int y, const int width, const int height) {
    ...
    const unsigned int values[] = {screen->black_pixel, eventValues};
    ...
    const xcb_intern_atom_cookie_t deleteCookie = xcb_intern_atom(connection, 0, 16, "WM_DELETE_WINDOW");
    const xcb_intern_atom_cookie_t protocolCookie = xcb_intern_atom(connection, 0, 12, "WM_PROTOCOLS");
    const xcb_intern_atom_reply_t *deleteReply = xcb_intern_atom_reply(connection, deleteCookie, nullptr);
    const xcb_intern_atom_reply_t *protocolReply = xcb_intern_atom_reply(
        connection, protocolCookie, nullptr);
    wm_delete_win = deleteReply->atom;
    wm_protocols = protocolReply->atom;
    ...
    if (const int streamResult = xcb_flush(connection); streamResult <= 0) {
...
void *PlatformLinux::allocate(const unsigned long size, bool align) {
    return malloc(size);
}

void PlatformLinux::freeMemory(void *memory, bool align) {
    free(memory);
}

void PlatformLinux::clear(void *memory, const unsigned long size) {
